...
import { Flame, Music2, User, LogOut, Home, BookOpen, Wind, CheckCircle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 pb-20 md:pb-0">
      ...
      <main className="container mx-auto px-4 py-8 space-y-8">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-3xl font-bold">Músicas Mindful</h2>
          {isKeyUser && (
            <Button onClick={() => navigate('/editor/music')}>
              Adicionar Música
            </Button>
          )}
        </div>
                ...
                >
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex justify-between items-center text-xs text-muted-foreground">
                      ...
                      <span>Lançamento: {new Date(item.release_timestamp).toLocaleDateString()}</span>
                    </div>
                    <div className="flex gap-2 mt-4">
                      <Button size="sm" className="flex-1" onClick={() => navigate(`/music/${item.id}`)}>
                        Ouvir
                      </Button>
                      {isKeyUser && (
